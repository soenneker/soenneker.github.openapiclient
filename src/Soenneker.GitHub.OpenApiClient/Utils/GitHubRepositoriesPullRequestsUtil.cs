...
public sealed class GitHubRepositoriesPullRequestsUtil : IGitHubRepositoriesPullRequestsUtil
{
        ...
        DateTime? endAt = null, bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        var page = 1;
        IReadOnlyList<PullRequestSimple> pullRequests;

        do
        {
            pullRequests = await client.Repos[owner][name].Pulls.GetAsync(
            ...
            page++;
        } while (pullRequests.Count > 0 && !cancellationToken.IsCancellationRequested);
        ...
        bool log = false, CancellationToken cancellationToken = default)
    {
        ...
        var repositories = await _gitHubRepositoriesUtil.GetAllForOwner(owner, startAt, endAt, cancellationToken);
        var result = new List<PullRequest>();

        foreach (var repository in repositories)
        {
            var pullRequests = await GetAll(repository.Owner.Login, repository.Name, username, startAt, endAt, false, cancellationToken);
            result.AddRange(pullRequests);
        }

        if (log)
            _logger.LogInformation("Found {count} PRs for owner {owner}", result.Count, owner);
        ...
        DateTime? endAt = null, bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        var pullRequests = await GetAll(owner, name, username, startAt, endAt, false, cancellationToken);
        var result = new List<PullRequest>();

        foreach (var pr in pullRequests)
        {
            bool isApproved = await IsApproved(owner, name, pr.Number ?? 0, cancellationToken);
        ...
        bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var repository in repositories)
        {
            var pullRequests = await GetAllNonApproved(repository.Owner.Login, repository.Name, username, startAt, endAt, false, cancellationToken);
            result.AddRange(pullRequests);
        }
    ...
    public async ValueTask<bool> IsApproved(string owner, string repo, int pullRequestNumber, CancellationToken cancellationToken = default)
    {
        ...
        var reviews = await client.Repos[owner][repo].Pulls[pullRequestNumber].Reviews.GetAsync(cancellationToken: cancellationToken);
        ...
        int delayMs = 0, CancellationToken cancellationToken = default)
    {
        ...
        var pullRequests = await GetAllNonApproved(owner, name, username, startAt, endAt, false, cancellationToken);
        ...
        DateTime? endAt = null, bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var repository in repositories)
        {
            var pullRequests = await GetAll(repository, null, startAt, endAt, false, cancellationToken: cancellationToken);
        ...
        DateTime? endAt = null, bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var repository in repositories)
        {
            ...
            foreach (var pr in pullRequests)
            {
                bool hasFailedBuild = await _gitHubRepositoriesRunsUtil.HasFailedRun(repository, pr, cancellationToken);
        ...
        DateTime? endAt = null, bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        var repositories = await _gitHubRepositoriesUtil.GetAllForOwner(owner, startAt, endAt, cancellationToken);
        var convertedRepositories = ConvertToRepositories(repositories);
        var repositoriesWithOpenPrs = await FilterRepositoriesWithOpenPullRequests(convertedRepositories, startAt, endAt, false, cancellationToken);
        var result = await FilterRepositoriesWithFailedBuilds(repositoriesWithOpenPrs, startAt, endAt, log, cancellationToken);
        ...
        bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        var repositories = await _gitHubRepositoriesUtil.GetAllForOwner(owner, startAt, endAt, cancellationToken);
        var convertedRepositories = ConvertToRepositories(repositories);
        var result = await FilterRepositoriesWithOpenPullRequests(convertedRepositories, startAt, endAt, log, cancellationToken);
